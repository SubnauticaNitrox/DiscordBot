...
public class AutoResponseService : DiscordBotHostedService
{
    ...
    public IEnumerable<AutoResponseConfig.Definition> Definitions => options.CurrentValue.AutoResponseDefinitions;
    ...
    {
        foreach (AutoResponseConfig.Definition definition in Definitions)
        {
            if (!MatchesFilters(definition.Filters, author, message)) continue;

            foreach (AutoResponseConfig.Response response in definition.Responses)
            {
    ...
    private bool MatchesFilters(AutoResponseConfig.Filter[] filters, SocketGuildUser author, SocketUserMessage message)
    {
        foreach (AutoResponseConfig.Filter filter in filters)
        {
            switch (filter.Type)
            {
                case "Channel" when filter.Value is string valueStr && ulong.TryParse(valueStr, out ulong channelId):
                    if (message.Channel.Id != channelId) return false;
                    break;
                case "UserJoinTimeSpan" when filter.Value is string valueStr &&
                                             TimeSpan.TryParse(valueStr, out TimeSpan valueTimeSpan):
                    if (DateTimeOffset.UtcNow - author.JoinedAt > valueTimeSpan) return false;
                    break;
                case "MessageWordOrder" when filter.Values is [..] values:
